...
> **Goal:** Use the Datakit plugin to orchestrate multi-step API workflows - fetch an OAuth token from a third-party IdP, inject it into the upstream request, and transform the response - all within a single Kong plugin.

::: warning Enterprise
`datakit` requires **Kong Gateway Enterprise** or **Konnect**. Min version: Kong Gateway 3.11.
:::
